...
class ConverterCapecToJson(Converter):
    """转换器
    将Csv格式的Capec数据集转换成便于
    提取器Extractor处理的格式
    """
    ...
    def split_property(self, objects: list) -> list:
        for object in objects:
            ...
            # # 将其作为属性,用于关系链接
            object["Consequences"] = self.split_consequences(
                object.get("Consequences", []))
            for name in ["Related Attack Patterns", "Taxonomy Mappings",
                         "Execution Flow", "Skills Required", "Resources Required",
                         "Notes", "Consequences"]:
                object[name] = self.split_colon(
                    object.get(name, []))
            for o in object["Taxonomy Mappings"]:
                name = o.get("TAXONOMY NAME", "")
                if name != "":
                    if name == "ATTACK" and "ENTRY ID" in o:
                        o["ENTRY ID"] = "T" + o["ENTRY ID"]
                    p_name = f"r_{name}"
                    if p_name not in object:
                        object[p_name] = []
                    object[p_name].append(o)
        pass
    ...
    def split_consequences(self, object_list: list) -> list:
        ...
        for line in object_list:
            position = line.find("TECHNICAL IMPACT:")
            a_str = ":" + line[position:]
            for part in line[:position].replace(":SCOPE", "::SCOPE").split("::"):
    ...
    def convert(self, file_path: str) -> json:
        ...
        objects_dict = {}
        objects_dict['capec'] = self.preprocess(dataf).to_dict('records')
        self.split_property(objects_dict['capec'])
