...
class Cnvdspider(object):
    def __init__(self):
        self.headers = {
            "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.16 "
                          "Safari/537.36"}
    ...
    def get_cookies(self):
        ...
        # 可能需要手动添加路径
        driver = webdriver.Chrome("C:\\Users\\8208191402\\Desktop\\研究生\\chromedriver.exe",chrome_options=chrome_options)
    ...
    def parse(self, i):
        ...
        if (self.count == 5):
            self.cookies = self.get_cookies()
            self.count = 0
        url = 'https://www.cnvd.org.cn/flaw/list.html?%s' % str(i)
